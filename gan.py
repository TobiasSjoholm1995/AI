...
def show_result(generator, discriminator, count = 1):
   gen_images, gen_scores = generate_images(generator, discriminator, count)
   gen_names = [f'Gen_Result_{i}_Score_{(int(round(100 * s)))}.jpg' for i, s in enumerate(gen_scores)]
...
def generate_images(generator, discriminator, count):
   ...
   scores = discriminator.predict(images, verbose=0).flatten()
...
def main():
   ...
   show_result(generator, discriminator, 10)
